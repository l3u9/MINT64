...
{
    int i, j;
    BOOL bPreviousInerrupt;
    BOOL bResult;
    BYTE bData;

    bPreviousInerrupt = kSetInterruptFlag(FALSE);
    ...
    if(bResult == FALSE)
    {
        kSetInterruptFlag(bPreviousInerrupt);
        return FALSE;
    }
...
{
    kInitializeQueue(&gs_stKeyQueue, gs_vsKeyQueueBuffer, KEY_MAXQUEUECOUNT, sizeof(KEYDATA));
