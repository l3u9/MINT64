...
void kPrintMPConfigurationTable(void)
{
    ...
    QWORD qwMPFloatingPouinterAddress;
    MPFLOATINGPOINTER* pstMPFloatingPointer;
    ...
    char* vpcInterruptFlagsP0[4] = {"Conform", "Active High", "Reserved", "Active Low"};
    ...
    for(i = 0; i < pstMPTableHeader->wEntryCount; i++)
    {
        ...
        switch(bEntryType)
        {
            ...
            case MP_ENTRYTYPE_IOINTERRUPTASSIGNMENT:
                ...
                kPrintf("(%s)\n", vpcInterruptFlagsEL[pstIOAssignmentEntry->bInterruptType]);
                kPrintf("I/O Interrupt flags : 0x%X", pstIOAssignmentEntry->wInterruptFlags);
                kPrintf("(%s, %s)\n", vpcInterruptFlagsP0[pstIOAssignmentEntry->wInterruptFlags & 0x3], vpcInterruptFlagsEL[(pstIOAssignmentEntry->wInterruptFlags >> 2) & 0x03]);
            ...
            case MP_ENTRYTYPE_LOCALINTERRUPTASSIGNMENT:
                ...
                kPrintf("(%s, %s)\n", vpcInterruptFlagsP0[pstLocalAssignmentEntry->wInterruptFlags & 0x03], vpcInterruptFlagsEL[(pstLocalAssignmentEntry->wInterruptFlags >> 2) & 0x03]);
