...
SHELLCOMMANDENTRY gs_vstCommandTable[] =
{
        ...
        { "filesysteminfo", "Show FileSystem Information", kShowFileSystemInformation},
        ...
        { "starttaskloadbal", "Start Task Load Balancingg", kStartTaskLoadBalancing},
...
static void kHelp( const char* pcCommandBuffer )
{
    ...
    for( i = 0 ; i < iCount ; i++ )
    {
        ...
        if((i != 0) && ((i% 20) == 0))
        {
            kPrintf("Press any key yo continue... ('q' is exit): ");
            if(kGetCh() == 'q')
            {
                kPrintf("\n");
                break;
...
static void kDeleteFileInRootDirectory(const char* pcParameterBuffer)
{
    ...
    {
        kPrintf("File Not Fount or File Opened");
        return;
    }
...
static void kDownloadFile(const char* pcParameterBuffer)
{
    ...
    while(dwReceivedSize < dwDataLength)
    {
        ...
        if(dwTempSize != 0)
        {
            //데이터가 수신된 것이 있으면 ACK 또는 파일 쓰기 수행
...
static void kStartSymmetricIOMode(const char* pcParamterBuffer)
{
