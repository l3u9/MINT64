...
static void kDrawDigitalClock(QWORD qwWindowID)
{
    RECT stWindowArea;
    RECT stUpdateArea;
    static BYTE s_bPreviousHor, s_bPreviousMinutes, s_bPreviousSecond;
    BYTE bHour, bMinute, bSecond;
    char vcBuffer[10] = "00:00 AM";

    kReadRTCTime(&bHour, &bMinute, &bSecond);

    if((s_bPreviousHor == bHour) && (s_bPreviousMinutes == bMinute) && (s_bPreviousSecond == bSecond))
        return;
    
    s_bPreviousHor = bHour;
    s_bPreviousMinutes = bMinute;
