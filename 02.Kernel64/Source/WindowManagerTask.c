...
//     WINDOWMANAGER* pstNextWindowEvent;
//     QWORD qwWindowID;
//     RECT stArea;
...
//             (vstEvent[i].qwType != EVENT_WINDOWMANAGER_UPDATESCREENBYSCREENAREA)) || (pstWindowEvent->qwWindowID != pstNextWindowEvent->qwMovingWindowID))
//                 continue;
            
//             if(kGetOverlappedRectangle(&(pstWindowEvent->stArea), &(pstNextWindowEvent->stScreenArea), &stOverlappedArea) == FALSE)
//                 continue;
            
//             if(kMemCmp(&(pstWindowEvent->stArea), &stOverlappedArea, sizeof(RECT)) == 0)
//             {
//                 kMemCpy(&(pstWindowEvent->stArea), &(pstWindowEvent->stArea), sizeof(RECT));
//                 vstEvent[i].qwType = EVENT_UNKNOWN;
//             }
//             else if(kMemCmp(&(pstNextWindowEvent->stScreenArea), &stOverlappedArea, sizeof(RECT)) == 0)
//                 vstEvent[i].qwType = EVENT_UNKNOWN;
//         }
//     }
