...
static BOOL kLoadProgramAndRelocation(BYTE* pbFileBuffer, QWORD* pqwApplicationMemoryAddress, QWORD* pqwApplicationMemorySize, QWORD* pqwEntryPointAddress)
{
    ...
    kPrintf("Magic Number [%c%c%c] Section Header Count [%d]\n", 
        pstELFHeader->e_ident[1], pstELFHeader->e_ident[2], pstELFHeader->e_ident[3],
         pstELFHeader->e_shnum);
    kPrintf("File Type[%d]\n", pstELFHeader->e_type);
    ...
    kPrintf("Section Name String Table Section Inde [%d]\n", pstELFHeader->e_shstrndx);
    ...
    kPrintf("Aligned Meory Size [0x%q]\n", qwMemorySize);
    ...
    for(i = 1; i < pstELFHeader->e_shnum; i++)
    {
        ...
        if(pstSectionHeader[i].sh_type == SHT_NOBITS)
            ...
            kMemCmp(pstSectionHeader[i].sh_addr, pbFileBuffer + pstSectionHeader[i].sh_offset, 
                pstSectionHeader[i].sh_size);
        
        kPrintf("Section [%x] Virtual Address [%q] File Address [%q] Size [%q]\n", i, 
            pstSectionHeader[i].sh_addr, pbFileBuffer + pstSectionHeader[i].sh_offset, 
            pstSectionHeader[i].sh_size);
    }
...
static BOOL kRelocation(BYTE* pbFileBuffer)
{
    ...
    for( i = 1; i < pstELFHeader->e_shnum; i++)
    {
        ...
        for(j = 0; j < pstSectionHeader[i].sh_size; )
        {
            ...
            if(pstSymbolTable[RELOCATION_UPPER32(ulInfo)].st_shndx == SHN_COMMON)
            {
