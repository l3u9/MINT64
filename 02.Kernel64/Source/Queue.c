...
void kInitializeQueue(QUEUE* pstQueue, void* pvQueueBuffer, int iMaxDataCount, int iDataSize)
{
    ...
    pstQueue->bLastPoerationPut = FALSE;
}


BOOL kIsQueueFull(const QUEUE* pstQueue)
{
    if((pstQueue->iGetIndex == pstQueue->iPutIndex) && (pstQueue->bLastPoerationPut == TRUE))
    {
        return TRUE;
    }
    return FALSE;
...
{
    if((pstQueue->iGetIndex == pstQueue->iPutIndex) && (pstQueue->bLastPoerationPut ==  FALSE))
    {
        return TRUE;
    }
    return FALSE;
...
BOOL kPutQueue(QUEUE* pstQueue, const void* pvData)
{
    ...
    pstQueue->bLastPoerationPut = TRUE;
