...
BOOL kMount(void)
{
    ...
    // kPrintf("dwReservedSectorCount: %d\ndwClusterLinkAreaStartAddress: %d\ndwClusterLinkAreaSize: %d\ndwDataAreaStartAddress: %d\ndwTotalClusterCount: %d\n",pstMBR->dwReservedSectorCount, pstMBR->dwReservedSectorCount + 1, pstMBR->dwClusterLinkSectorCount, pstMBR->dwReservedSectorCount + pstMBR->dwClusterLinkSectorCount + 1, pstMBR->dwTotalClusterCount);
...
BOOL kFormat( void )
{
    ...
    // kPrintf("dwTotalSectorCount: %d\ndwMaxClusterCount: %d\ndwClusterLinkSectorCount: %d\ndwRemainSectorCount: %d\ndwClsuterCount: %d\ndwClusterLinkSectorCount: %d\n", dwTotalSectorCount, dwMaxClusterCount, dwClusterLinkSectorCount, dwRemainSectorCount, dwClsuterCount);
        ...
        if(pstCacheBuffer->bChanged == TRUE)
        {
            switch(iCacheTableIndex)
            {
                ...
                case CACHE_DATAAREA:
                    ...
                    {
                        kPrintf("Cache Bufer Write Fail\n");
                        return NULL;
                    }
                    break;
                default:
...
DIR* kOpenDirectory(const char* pcDirectoryName)
{
    ...
    //루트 디렉토리 밖에 없어서 디렉토리 이름을 무시하고 핸들만 할당
